...
# This script subscribes to the Fast FoundationStereo disparity output
# and visualizes or saves it as a colormap image.
...
def get_args():
    parser = argparse.ArgumentParser(
        description='Fast FoundationStereo Disparity Node Visualizer')
    ...
    parser.add_argument('--left_image_path',
                        default=ros_ws +
                        '/src/fast-foundationstereo/demo_data/left.png',
                        help='Absolute path your left image.')
    parser.add_argument('--right_image_path',
                        default=ros_ws +
                        '/src/fast-foundationstereo/demo_data/right.png',
                        help='Absolute path your right image.')
    parser.add_argument('--camera_info_path',
                        default=ros_ws +
                        '/isaac_ros_assets/isaac_ros_foundationstereo/examples/camera.json',
                        help='Absolute path your camera info json file.')
    args = parser.parse_args()
    return args


class FastFoundationStereoVisualizer(Node):

    def __init__(self, args):
        super().__init__('fast_foundationstereo_visualizer')
        self.args = args
        self.encoding = 'rgb8'
    ...
    def _prepare_raw_inputs(self):
        ...
        self.left_msg = self._bridge.cv2_to_imgmsg(
            np.array(left_img), self.encoding)
        self.right_msg = self._bridge.cv2_to_imgmsg(
            np.array(right_img), self.encoding)
    ...
    def disparity_callback(self, disp_msg):
        ...
        disp_img = (disp_img - self.args.min_disp) / \
            (self.args.max_disp - self.args.min_disp)
        disp_img *= 255
        color_map = cv2.applyColorMap(
            disp_img.astype(np.uint8), cv2.COLORMAP_VIRIDIS)
        if self.args.save_image:
            cv2.imwrite(self.args.result_path, color_map)
        else:
            cv2.imshow('fast_foundationstereo_output', color_map)
        cv2.waitKey(1)
...
def main():
    args = get_args()
    rclpy.init()
    rclpy.spin(FastFoundationStereoVisualizer(args))
    rclpy.shutdown()
